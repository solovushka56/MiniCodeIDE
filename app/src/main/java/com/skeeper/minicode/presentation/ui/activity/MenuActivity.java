...
public class MenuActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getWindow().setNavigationBarColor(getResources().getColor(R.color.gradient_dark));
        ...
        binding.projectsButton.setOnClickListener(v -> {
            ...
            switchActiveButton((MaterialButton) v);
        });
    ...
    private void setActiveButton(MaterialButton button) {
        ...
        button.setIconTint(ColorStateList.valueOf(getColor(R.color.blue_light)));
    }

    private void switchActiveButton(MaterialButton clickedButton) {
    ...
    private void animateButton(MaterialButton button, boolean activate) {
        float targetScale = activate ? 1.0f : 0.8f;

        int bgColor = activate
                ? getColor(R.color.violet_light)
                : getColor(R.color.violet);
        int contentColor = activate
        ...
        TimeInterpolator scaleInterpolator = input -> BackInterpolations.backInOut(input);
        scaleX.setInterpolator(scaleInterpolator);
        scaleY.setInterpolator(scaleInterpolator);

        int currentBgColor = button.getBackgroundTintList() != null
                ? button.getBackgroundTintList().getDefaultColor()
                : Color.TRANSPARENT;

        ValueAnimator bgAnimator = ValueAnimator.ofArgb(currentBgColor, bgColor);
        bgAnimator.addUpdateListener(anim ->
                button.setBackgroundTintList(ColorStateList.valueOf((int) anim.getAnimatedValue()))
        );

        int currentIconColor = button.getIconTint() != null
                ? button.getIconTint().getDefaultColor()
                : Color.WHITE;

        var iconAnimator = ValueAnimator.ofArgb(currentIconColor, contentColor);
        iconAnimator.addUpdateListener(anim ->
                button.setIconTint(ColorStateList.valueOf((int) anim.getAnimatedValue()))
        );

        int currentTextColor = button.getCurrentTextColor();
        var textAnimator = ValueAnimator.ofArgb(currentTextColor, contentColor);
        textAnimator.addUpdateListener(anim ->
                button.setTextColor((int) anim.getAnimatedValue())
        );

        var animatorSet = new AnimatorSet();
        animatorSet.playTogether(scaleX, scaleY, bgAnimator, iconAnimator, textAnimator);
